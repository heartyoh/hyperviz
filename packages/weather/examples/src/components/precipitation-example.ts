...
import { PrecipitationLayer } from "../../../src/layers/precipitation-layer.js";
 ...
 * 강수량 예제 컴포넌트
 */
export class PrecipitationExample {
  // OpenLayers 지도 객체
  private map: Map | null = null;
  // 강수량 레이어
  private precipitationLayer: PrecipitationLayer | null = null;
  // 날씨 서비스
  private weatherService: WeatherService | null = null;
  // 강수량 레이어 옵션
  private precipitationOptions = {
    minPrecipitation: 0,
    maxPrecipitation: 100,
    opacity: 0.7,
    interpolation: "bilinear" as "linear" | "bilinear" | "bicubic" | undefined,
    colorScale: [
      "rgba(240, 240, 255, 0.1)", // 매우 약한 비
      "rgba(190, 190, 255, 0.3)", // 약한 비
      "rgba(120, 120, 255, 0.5)", // 중간 비
      "rgba(30, 30, 255, 0.7)", // 강한 비
      "rgba(0, 0, 180, 0.9)", // 폭우
    ],
  };
  // 상태 표시 요소
  ...
  constructor(
    ...
    Object.entries(options).forEach(([key, value]) => {
      if (value !== undefined) {
        (this.precipitationOptions as any)[key] = value;
      }
    });

    logDebug("강수량 예제 생성됨", {
      settings,
      options: this.precipitationOptions,
    });
  }
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      logDebug("강수량 예제 초기화 완료");
    } catch (error) {
      console.error("강수량 예제 초기화 실패:", error);
      this.showError("강수량 예제 초기화 실패");
    }
  }
    ...
    try {
      ...
      weatherService
        ...
        .then((weatherData) => {
          ...
          // 강수량 레이어 생성 및 추가
          this.initializePrecipitationLayer(weatherData);
          ...
          weatherService.subscribe((newWeatherData) => {
            ...
            if (this.precipitationLayer) {
              this.precipitationLayer.setWeatherData(
   ...
   * 강수량 레이어 초기화
   */
  private initializePrecipitationLayer(weatherData: WeatherDataBase[]): void {
    try {
      // 이전 레이어가 있으면 제거
      if (this.precipitationLayer) {
        this.precipitationLayer.dispose();
      ...
      this.precipitationLayer = new PrecipitationLayer({
        colorScale: this.precipitationOptions.colorScale,
        minPrecipitation: this.precipitationOptions.minPrecipitation,
        maxPrecipitation: this.precipitationOptions.maxPrecipitation,
        opacity: this.precipitationOptions.opacity,
        interpolation: this.precipitationOptions.interpolation,
      });
      ...
      this.precipitationLayer.addToMap(this.map);
      ...
      logDebug("강수량 레이어 초기화 완료");

      // UI 컨트롤 생성
      this.createControls();
    } catch (error) {
      logDebug("강수량 레이어 초기화 실패", error);
      throw new Error("강수량 레이어 초기화 실패: " + error);
    }
  }
  ...
  private createControls(): void {
    ...
    const controlsContainer = createElement("div", { class: "controls" }, [
      ...
      this.createControlGroup(
        "강수량 레이어 표시",
        "checkbox",
        "precipitationLayerToggle",
        "true",
        (value) => {
          if (this.precipitationLayer) {
            this.precipitationLayer.setVisible(value === "true");
          }
        }
      ),
      ...
      this.createSliderGroup(
        "최소 강수량",
        "minPrecipitation",
        this.precipitationOptions.minPrecipitation.toString(),
        "0",
        "50",
        "1",
        " mm",
        (value) => {
          this.precipitationOptions.minPrecipitation = parseFloat(value);
      ...
      this.createSliderGroup(
        "최대 강수량",
        "maxPrecipitation",
        this.precipitationOptions.maxPrecipitation.toString(),
        "50",
        "200",
        "5",
        " mm",
        (value) => {
          this.precipitationOptions.maxPrecipitation = parseFloat(value);
          this.updatePrecipitationLayer();
        }
      ),
      ...
      this.createSliderGroup(
        "불투명도",
        "opacity",
        this.precipitationOptions.opacity.toString(),
        "0.1",
        "1",
        "0.05",
        "",
        (value) => {
          this.precipitationOptions.opacity = parseFloat(value);
          this.updatePrecipitationLayer();
        }
      ),
      ...
      this.createControlGroup(
        ...
        "select",
        "interpolation",
        this.precipitationOptions.interpolation || "bilinear",
        (value) => {
          this.precipitationOptions.interpolation = value as
            | "linear"
            | "bilinear"
            | "bicubic"
            | undefined;
  ...
  private updatePrecipitationLayer(): void {
    if (!this.precipitationLayer || !this.map) return;

    logDebug("강수량 레이어 옵션 업데이트", this.precipitationOptions);

    // 현재 레이어 로컬 참조 생성
    const currentLayer = this.precipitationLayer;
